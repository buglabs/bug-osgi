...
public class LandmarkStore {
	/*
	 * Overview of implementation (for JSR-179 hackers interested in the guts)
	 * The Landmark store is implemented as a RecordStore (part of the MIDP
	 * specification). The LandmarkStore name is prefixed by RECORD_STORE_PREFIX
	 * to create the RecordStore name on disc. This is so that we don't clash
	 * with other stores using the same name in completely unrelated stores for
	 * other applications. We never save permanent temporary local versions of
	 * the Landmarks as we don't want to have to store them all in memory.
	 * WeakReferences are used to keep track of instances. This may have the
	 * strange side effect that Landmarks may be returned that have been edited
	 * by the same program but not saved to disc. e.g. An application gets all
	 * Landmarks from the store and edits the Landmark for "Edinburgh Castle"
	 * and changes it's name to "Scottish Castle". The same application then
	 * emmediately requests all landmarks in Edinburgh. The returned Landmark
	 * will include the edited one, not the one on disc. It will not be saved to
	 * disc until .updateLandmark() is called on it. Permission-wise, the
	 * default store is editable by all but new stores are only writable by the
	 * app that created them in the first place. LandmarkStores may be read by
	 * other apps. Note that the spec does not require additional stores to be
	 * supported by the implementation and there is no standard policy regarding
	 * the permission of such stores. This policy was chosen as it seems the
	 * most sensible. Due to the lack of Serialization support in J2ME, this
	 * contains private static methods to serialise and deserialise the Landmark
	 * objects. Included in the serialisation is an unspecified number of
	 * category Strings. We define a class named CategorisedLandmark which is
	 * simply a container class for a Landmark and an array of String
	 * categories, it is these that are actually serialised. Every LandmarkStore
	 * has a record that holds the valid categories. Unfortunately the
	 * RecordStore spec does not allow writing to an arbitrary record ID if it
	 * doesn't exist (MPowerPlayer certainly does not allow this), nor is there
	 * a standard on the ID of the first record (various implementations use 0
	 * or 1). It gets worse... the RecordEnumeration does not quarantee that its
	 * first entry will be the first record, so each new instance of
	 * LandmarkStore must first find out which record holds the category
	 * information. The only way to do this is to attempt to read all the
	 * records until one is only Strings, then record this ID in a local
	 * variable. This ID will never change unless something evil accesses the
	 * RecordStore directly or the implementation is completely moronic and
	 * changes record IDs on the fly. The first (ID unspecified) record in the
	 * store will always be a simple array of valid category Strings. By storing
	 * this information in the RecordStore and not locally in the instance of
	 * the LandmarkStore, it helps to keep the store syncronised across
	 * different applications using the same store at the same time, although
	 * due to the nature of the RecordStore, syncronisation cannot be guaranteed
	 * and there will always be classic database race conditions. Future work
	 * may involve making the get* methods memoryless. The spec requires that
	 * they return Enumerations over Landmark objects. Currently we load all the
	 * Landmark objects and return them as a live Enumeration, however a more
	 * scalable solution would involve only reading the record from the disc
	 * when requested. In terms of compliance with the spec... this does a good
	 * job, but because of the lack of SecurityException throws in RecordStore
	 * for read operations, most security-related denials of service will be
	 * reported as IOExceptions.
	 */

	/**
	 * For the purposes of serialisation of the AddressInfo object, this is
	 * shared between {@link #serialiseLandmark(Landmark)} and
	 * {@link #landmarkFromSerialised(byte[])}
	 */
	private static final int[] ADDRESS_INFO_ORDER = { AddressInfo.BUILDING_FLOOR, AddressInfo.BUILDING_NAME, AddressInfo.BUILDING_ROOM, AddressInfo.BUILDING_ZONE,
			AddressInfo.CITY, AddressInfo.COUNTRY, AddressInfo.COUNTRY_CODE, AddressInfo.COUNTY, AddressInfo.CROSSING1, AddressInfo.CROSSING2, AddressInfo.DISTRICT,
			AddressInfo.EXTENSION, AddressInfo.PHONE_NUMBER, AddressInfo.POSTAL_CODE, AddressInfo.STATE, AddressInfo.STREET, AddressInfo.URL };

	/**
	 * It is not possible to save a null string, so we use this as a
	 * placeholder. It is important that null strings be recovered otherwise pre
	 * and post stored objects will not have the same fields. Note that this
	 * means that the literal String "(null)" will be recovered as a null, but
	 * such a situation should be incredibly rare.
	 */
	private static final String NULL_STRING = "(null)";

	/**
	 * The LandmarkStore name is prefixed by this in order to get the name of
	 * the RecordStore we save on the device. #see
	 * {@link #recordStoreName(String)}
	 */
	private static final String RECORD_STORE_PREFIX = "jsr179_";

	/**
	 * The specification defines when to throw exceptions for filenames that are
	 * too long, but not a minimal length. However, the RecordStore imposes a 32
	 * char maximum.
	 */
	 ...
	 *             stores
	 */
	public static void createLandmarkStore(String storeName) throws NullPointerException, IllegalArgumentException, IOException, LandmarkException, SecurityException {
	 ...
	 * @throws SecurityException
	 *             if the application does not have permissions to delete a
	 *             landmark store
	 * @throws LandmarkException
	 *             if the implementation does not support deleting landmark
	 *             stores
	 */
	public static void deleteLandmarkStore(String storeName) throws NullPointerException, IOException, SecurityException, LandmarkException {
		// test Security permissions
		OpenLAPICommon.testPermission("javax.microedition.location.LandmarkStore.management");
	 ...
	 * 
	 * @param storeName
	 *            the name of the landmark store to open. if null, the default
	 *            landmark store will be returned
	 * @return the LandmarkStore object representing the specified landmark
	 *         store or null if a landmark store with the specified name does
	 *         not exist.
	 * @throws SecurityException
	 *             if the application does not have a permission to read
	 *             landmark stores
	 */
	public static LandmarkStore getInstance(String storeName) throws SecurityException {
		// test Security permissions
		OpenLAPICommon.testPermission("javax.microedition.location.LandmarkStore.read");

		/*
		 * Create a new LandmarkStore object and return it. If it has not been
		 * created return null, if the permissions are not correct throw a
		 * SecurityException, if there was an IO error return null.
		 */
		try {
	...
	public static String[] listLandmarkStores() throws SecurityException, IOException {
		...
		for (int i = 0; i < allStores.length; i++) {
			...
			if (storeName.startsWith(RECORD_STORE_PREFIX)) {
				...
				vecLandmarkStores.addElement(storeName.substring(RECORD_STORE_PREFIX.length()));
			}
		}
	...
	/**
	 * A private version of {@link #createLandmarkStore(String)} that can create
	 * the default store (null).
	 * 
	 * @param storeName
	 * @throws IllegalArgumentException
	 * @throws IOException
	 ...
	 * @throws SecurityException
	 */
	private static void createLandmarkStore2(String storeName) throws IllegalArgumentException, IOException, LandmarkException, SecurityException {
		...
		try {
			...
			if (storeName == null) {
				...
				store = RecordStore.openRecordStore(recordStoreName(storeName), true);
		 ...
		 * getInstance() will return one from the RecordStore we just created on
		 * the device.
		 */
	}

	/**
	 * Helper method that takes raw byte data for a single Landmark object as
	 * input and returns a CategorisedLandmark object. This is sadly needed as
	 * neither Landmark nor its components implement Serializable in the
	 * specification. Not that it matters since ObjectStream doesn't exist in
	 * J2ME.
	 * 
	 ...
	 * @throws IOException
	 */
	private static CategorisedLandmark deserialise(byte[] rawBytes) throws IOException {
		...
		QualifiedCoordinates qualifiedCoordinates = new QualifiedCoordinates(latitude, longitude, altitude, horizontalAccuracy, verticalAccuracy);
		...
		Landmark landmark;
		landmark = new Landmark(name, description, qualifiedCoordinates, addressInfo);
	...
	public Enumeration getLandmarks(String category, double minLatitude, double maxLatitude, double minLongitude, double maxLongitude) throws IOException, IllegalArgumentException {
		...
		for (int i = 0; i < records.length; i++) {
			...
			if ((category == null) || clm.inCategory(category)) {
				...
				if (((minLongitude <= maxLongitude) && (longitude >= minLongitude) && (longitude <= maxLongitude))
