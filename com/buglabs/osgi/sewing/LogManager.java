...
/**
 * Simple abstraction of logging so that sewing can have
 * less dependencies on the BUG libraries
 * 
 * @author bballantine
 *
 */
public class LogManager {
	...
	public static synchronized void log(int level, String message, Throwable exception) {
		...
		if (logger != null) {
			if (exception == null) logger.log(level, message);
			else logger.log(level, message, exception);
		} else {
	...
	private static void printLog(int level, String message, Throwable exception) {
		System.out.println('[' + getLevelString(level) + ']' + ' ' +
							message + " " + exception.getMessage());
		if (level == LogService.LOG_DEBUG) exception.printStackTrace();
	}
