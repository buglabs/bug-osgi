...
/**
 * This is the main Sewing framework servlet class.
 * clients are expected to extend this abstract class and implement 
 * the <code>getControllerMap()</code> method. This class Servlet is 
 * then registered with the OSGi runtime using ISewingService.
 * 
 * Then, for each page a client would like to publish, the client must
 * create SewingController classes (typically as inner classes of the 
 * client's SewingHttpServlet implementation).  The url mapping (url -> controller) 
 * is done in the getControllerMap() method.
 * 
 * @author brian
 * 
 * UPDATE: 
 * 2010-07-14 akweon: added beforeGet() and beforePost() in processRequest
 *
 */
public abstract class SewingHttpServlet extends HttpServlet {
	 ...
	 * Your implementation needs to map the inner class controllers to a string name
	 * which is what shows up on the URL.
	 * 
	 * @return
	 */
	...
	public final void setup(
			BundleContext bundleContext, HttpService httpService, String servletAlias) {
		...
		try {
			...
			// register servlet + / so that if you end your url in a slash, it still works
			if (!servletAlias.endsWith("/"))
				httpService.registerServlet(servletAlias + '/', this, null, null);			
			
			// Register images
			httpService.registerResources(
				servlet_alias + "." + IMAGES_ALIAS, "",
				new ImagesHttpContext( bundle_context ));
			
			// Register stylesheets
			httpService.registerResources(
				servlet_alias + "." + STYLESHEET_ALIAS, "",
				new StylesheetsHttpContext( bundle_context ));
			
			// Register javascripts
			httpService.registerResources(
				servlet_alias + "." + JAVASCRIPT_ALIAS, "",
				new JavascriptsHttpContext( bundle_context ));			
	...
	/**
	 * Handles the GET request for SewingHttpServlet
	 * Do not override if you hope to use the framework as it was designed
	 */
	...
	/**
	 * Handles the POST request for SewingHttpServlet
	 * Do not override if you hope to use the framework as it was designed
	 */
	...
	private void processRequest(
			int type, HttpServletRequest req, HttpServletResponse resp) throws IOException {
	
		String controllerPath = req.getPathInfo();
		// set up controllerPath to be index if no controller specified in url
		if (controllerPath == null) controllerPath = "/index";
	...
	/**
	 * Broke down processRequest into a function that takes all of it's moving parts
	 * explicitly.
	 * 
	 * @param type
	 ...
	 */
	private void processRequest(
			int type, String controllerName, RequestParameters params, 
		...
		// and, though the servlet container on BUG is currently single-threaded,
		// in the future, there could be synchronization problems, especially with re-direct
		synchronized(controller) {
			try {
				...
				// check if the controller asked for a redirect and do it if redirect, root is ignored
				if (controller.doRedirect() &&
						controller.getRedirectInfo() != null) {
		...
		if (root == null) root = new SimpleHash();
	...
	private void processRequest(RedirectInfo redirectInfo) throws IOException {
		processRequest(redirectInfo.getRequestType(), 
				redirectInfo.getControllerName(), redirectInfo.getParams(), 
				redirectInfo.getRequestObject(), redirectInfo.getResponseObject());
	}

	
	private void handleRedirect(
			SewingController requestController, HttpServletResponse resp) throws IOException {
		// make sure we're not trying to redirect to self
		if (requestController.equals(getController(
				requestController.getRedirectInfo().getControllerName()))) {
	...
	/**
	 * Sets up some defaults in the templates that implementers 
	 * can use to access images, javascripts, etc.
	 * 
	 * @param root
	 */
	...
	private SewingController getController(String controllerName) {
		if (controller_map == null) controller_map = getControllerMap();
		if (controller_map == null) return null;
		return controller_map.get(controllerName);
	}
	
	private Template getTemplate(
			SewingController controller, String controllerName) throws IOException {
		String templateName = controller.getTemplateName();
		if (templateName == null) templateName = controllerName + TEMPLATE_EXT;
		
		URL templateUrl = bundle_context.getBundle().getResource(
				"/" + TEMPLATES_ALIAS + "/" + templateName);

		InputSource inputSource = 
			new InputSource(new InputStreamReader(templateUrl.openStream()));
	...
	private void renderError(int errorNum, HttpServletResponse resp) {
		...
		if (errorNum == 404) message = ERROR_404;
	...
	/**
	 *  images, javascripts, and stylesheets are assets
	 *  this HttpContext is implemented for each of the asset types
	 *  to properly server up the resource type from the right spot 
	 * 
	 * @author brian
	 *
	 */
