...
public class RequestHelper {
	 ...
	 * @return
	 */
	static public RequestParameters parseParamString(
			String param_string, RequestParameters param_map) {
	...
	public static boolean isMultipart(HttpServletRequest req) {
        ...
        return (contentType.length > 0 
        		&& contentType[0].trim().equals(MULTIPART_TYPE));
	}
	...
	public static String getMultipartBoundary(HttpServletRequest req) {
        ...
        int x = 1;
        while(x < contentType.length && 
        		!contentType[x].trim().startsWith(BOUNDARY_KEY)) x++;
        if (x == contentType.length) return null;
        
        // second string in item is boundaryItem
        // return it if it's found
        String[] boundaryItem = StringUtil.split(contentType[x].trim(), "=");
        
        if (boundaryItem.length == 2 
        		&& boundaryItem[0].trim().equals(BOUNDARY_KEY))
	...
	/**
	 * Read from a BufferedReader representing a multipart
	 * form post into a RequestParameters object
	 * 
	 * @param boundary
	 ...
	 * @throws IOException
	 */
	private static RequestParameters readMultipart(
			String boundary, HttpServletRequest req) throws IOException {
		RequestParameters params = new RequestParameters();
		...
		for (int i = 0; i < chunks.length; i++) {
			...
			if (chunks[i].trim().equals("--")) continue;
	...
	/**
	 * Reads a multipart chunk (which is a string)
	 * and returns a RequestParameters object with one parameter
	...
	/**
	 * Take in all the stuff that might make up a Request Parameter and 
	 * create a new RequestParameters object with only one param
	 * 
	 * @param name
	 * 	param name
	 * @param val
	 ...
	 * @return
	 * 	a new RequestParameters object w/ one item
	 */
	private static RequestParameters createParamsFromData(
			String name, String val, String filename, String contentType) {
		RequestParameters params = new RequestParameters();
	 ...
	 * @return
	 * 	the final composed data as a string
	 */
	 ...
	 * @return
	 */
	private static String getValFrom(
			String line, String prefix, String suffix) {
		...
		String out;
		if (end < 0) out = line.substring(start);
		else out = line.substring(start, end);
		return out;
	}
	
	
	/**
	 * This is a copy of kgilmer's split function from com.buglabs.util.StringUtil
	 * This function was copied over for the purpose of trying to remove 
	 * Buglabs framework code dependencies so it could potentially run in any osgi env.
	 * 
	 * @author bballantine
	 *
	 */
