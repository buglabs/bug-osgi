...
/**
 * An addition operator. Handles both String concatenation as well as numeric
 * arithmetic.
 * 
 * @version $Id: Plus.java 1123 2005-10-04 10:48:25Z run2000 $
 */
public final class Plus extends AbstractBinary implements Serializable {
	...
	/**
	 * Performs addition on the two expressions set previously. In the case of
	 * two numbers, one is added to the other using normal arithmetic, then
	 * returned. For Strings, or a combination of number and String, the results
	 * are calculated as Strings, and concatenated. If either of the values is
	 * neither a number nor a String, an exception is thrown.
	 * 
	 * @param modelRoot
	 *            the template model that will be evaluated by the expression
	 * @return A {@link freemarker.template.FastScalar} of the concatenated
	 *         models, or a {@link freemarker.template.FastNumber} of the added
	 *         models
	...
	public TemplateModel getAsTemplateModel(TemplateWriteableHashModel modelRoot) throws TemplateException {
		...
		String leftValue = ExpressionUtils.getAsString(leftModel);
		String rightValue = ExpressionUtils.getAsString(rightModel);

		if (leftValue != null) {
			if (rightValue != null) {
				StringBuffer output = new StringBuffer(leftValue.length() + rightValue.length());
				output.append(leftValue);
				output.append(rightValue);
				return new FastScalar(output.toString());
			}
			return new FastScalar(leftValue);
		} else if (rightValue != null) {
			return new FastScalar(rightValue);
		}
		return null;
	}
	...
	public int getPrecedence() {
		return ExpressionBuilder.PRECEDENCE_ADDITION;
	}
