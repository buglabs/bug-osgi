 ...
 * A division operator.
 * 
 * @version $Id: Divide.java 1123 2005-10-04 10:48:25Z run2000 $
 * @since 1.8
 */
public final class Divide extends AbstractBinary implements Serializable {

	/** Serialization UUID for this class. */
	private static final long serialVersionUID = 6424280578840481872L;
	...
	public void setLeft(Expression left) {
		if ((left.getType() & ExpressionUtils.EXPRESSION_TYPE_NUMBER) == 0) {
			throw new IllegalArgumentException("Numeric expression is required for divide operation");
		}
		super.setLeft(left);
	}
	...
	/**
	 * Performs division on the two number models set previously. One is divided
	 * by the other, truncating to the next lowest whole number, then returned.
	 * If either of the values is not a number, an exception is thrown.
	 * 
	 * @param modelRoot
	 *            the template model that will be evaluated by the expression
	 * @return A {@link FastNumber} of the expression
