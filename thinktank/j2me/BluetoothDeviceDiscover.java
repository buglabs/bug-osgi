	...
	private final Runnable runner = new Runnable() {
		public void run() {
			...
			try {
				...
				// TTUtils.log("BluetoothDeviceDiscover runner 1:" + e.getMessage());
				finish();
				return;
			}
			...
			try {
				...
				// TTUtils.log("BluetoothDeviceDiscover runner 2:" + e.getMessage());
				finish();
	...
	public Hashtable discover() {
		...
		synchronized (waiter) {
			try {
				...
				// interpret an interupt as an early exit
				TTUtils.log("BluetoothDeviceDiscover.discover was interrupted:" +
					e.getMessage());
			}
		}
		return devices;
